...
class TestPosts(TestCase):
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            username="jungo", email="test@sds.com", password="1234"
        )

    def test_get_posts_pages(self):
    ...
    def test_creating_posts(self):
        login = self.client.login(username="jungo", password="1234")
        self.assertTrue(login)

        url = reverse("posts:post_create")
        image = SimpleUploadedFile("test.jpg", b"whatevercontents")
        response = self.client.post(url, {"image": image, "caption": "test test"})
        self.assertEqual(response.status_code, 200)
